            ...
            <nav className={`px-12 py-4 fixed top-0 bg-primary w-full z-10 ${isOpen ? "menu w-auto" : ""}`}>
                <ul className="md:flex lg:flex gap-12">
                    <div className={`${isOpen ? "px-12 py-4" : ""}`}>
                        <li className="visible md:hidden lg:hidden text-4xl text-secundary absolute" onClick={toggleMenu}> 
                            <AiOutlineMenu />
                        </li>    
                        <li>
                            <h1 className="pl-12 md:pl-0 lg:pl-0 pb-2 md:pb-0 lg:pb-0 text-4xl md:text-3xl lg:text-3xl font-bebas text-secundary"> Gameflix </h1>
                        </li>                
                    </div>
                    <li>
                        <ul>
                            <li className={`flex gap-2 md:gap-5 lg:gap-5 pt-0 md:pt-2 lg:pt-2 text-lg md:text-sm lg:text-sm menu-items ${isOpen ? "flex-col open pl-12 pt-5" : ""}`}>
